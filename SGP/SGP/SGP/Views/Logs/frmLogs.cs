    ...
    public partial class frmLogs : Form
    {
        ...
        public void fillCombo()
        {
            try
            {
                ...
                cc.da.SelectCommand = new SqlCommand("SELECT distinct RTRIM(UserName) FROM Registration", cc.con);
        ...
        private void GetData()
        {
            try
            {
                cc.con = new SqlConnection(cs.DBConn);
                cc.con.Open();
                cc.cmd = new SqlCommand(@"SELECT RTRIM(R.UserName),RTRIM(L.Data) as Data,RTRIM(L.Operacao) from Logs L 
        ...
        public void DeleteRecord()
        {
            try
            {
                ...
                {
                    st1 = lblUser.Text;
                    st2 = "forma deletados todos os logs até '" + System.DateTime.Now + "'";
        ...
        private void cmbUserID_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                cc.con = new SqlConnection(cs.DBConn);
                cc.con.Open();
                cc.cmd = new SqlCommand(@"SELECT RTRIM(R.UserName) UserName,RTRIM(L.Data) Data,RTRIM(L.Operacao) Operacao from Logs L 
                                          INNER JOIN Registration R ON R.UserID = L.UserID
                                          where R.UserName='" + cmbUserID.Text + "' order by Data", cc.con);
                cc.rdr = cc.cmd.ExecuteReader(CommandBehavior.CloseConnection);
                dgw.Rows.Clear();
                while (cc.rdr.Read() == true)
                {
        ...
        private void btnGetData_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                cc.cmd = new SqlCommand(@"SELECT RTRIM(R.UserName),RTRIM(L.Data) as Data,RTRIM(L.Operacao) from Logs L 
                                          INNER JOIN Registration R ON R.UserID = L.UserID 
                                          where Date between @date1 and @date2 order by Data", cc.con);
